...
async function upsertFaculty(
  ...
  return existing !== null
    ? existing._id
    : await ctx.db.insert("faculties", { institutionId, name });
}
...
async function upsertDepartment(
  ...
  return existing !== null
    ? existing._id
    : await ctx.db.insert("departments", { facultyId, name });
}
...
async function upsertProgram(
  ...
  return existing !== null
    ? existing._id
    : await ctx.db.insert("programs", { departmentId, name });
}
...
async function upsertAcademicClass(
  ...
  return existing !== null
    ? existing._id
    : await ctx.db.insert("academicClasses", { programId, level, session });
}
...
async function upsertDivision(
  ...
  const divisions = await ctx.db
    .query("divisions")
    .withIndex("by_academicClassId", (q) =>
      q.eq("academicClassId", academicClassId),
    )
    .collect();
  const existing = divisions.find((division) => division.label === label);
  return existing !== undefined
    ? existing._id
    : await ctx.db.insert("divisions", { academicClassId, label });
}
...
async function resolveClass(
  ...
  if (faculty === null)
    throw new Error(
      `Run seedHierarchy first (missing faculty: ${facultyName})`,
    );
  const department = await findDepartment(ctx, faculty._id, departmentName);
  if (department === null)
    throw new Error(
      `Run seedHierarchy first (missing department: ${departmentName})`,
    );
  const program = await findProgram(ctx, department._id, programName);
  if (program === null)
    throw new Error(
      `Run seedHierarchy first (missing program: ${programName})`,
    );
  const academicClass = await ctx.db
    .query("academicClasses")
    .withIndex("by_program_level_session", (q) =>
      q.eq("programId", program._id).eq("level", level).eq("session", session),
    )
    .unique();
  if (academicClass === null) {
    throw new Error(
      `Run seedHierarchy first (missing class: ${programName} L${level} ${session})`,
    );
  }
...
async function upsertCourse(
  ...
  const existing = await findCourseByCode(
    ctx,
    semesterId,
    academicClassId,
    courseCode,
...
async function upsertCourseSection(
  ...
  const existing = sections.find(
    (section) => section.divisionId === divisionId,
  );
...
async function upsertReminderPreferences(
  ...
  const existing = await ctx.db
    ...
    .withIndex("by_studentId_and_priority", (q) =>
      q.eq("studentId", studentId).eq("priority", priority),
    )
    .unique();
...
type PersonalReminderSeed = Omit<
  Doc<"personalReminders">,
  "_id" | "_creationTime" | "isCompleted"
>;

async function upsertPersonalReminder(
  ctx: MutationCtx,
  reminder: PersonalReminderSeed,
) {
  const existing = await ctx.db
    .query("personalReminders")
    .withIndex("by_userId_and_semesterId", (q) =>
      q.eq("userId", reminder.userId).eq("semesterId", reminder.semesterId),
    )
    .collect();
...
export const seedInstitution = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    return ctx.db.insert("institutions", {
      ...
      emailDomain: INSTITUTION_EMAIL_DOMAIN,
    });
  },
});
...
export const seedHierarchy = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const csHnd200 = await upsertAcademicClass(
      ctx,
      csHndId,
      DEMO_LEVEL,
      DEMO_SESSION,
    ...
    const csBTechId = await upsertProgram(
      ctx,
      csDeptId,
    ...
    const mathDeptId = await upsertDepartment(
      ctx,
      fastId,
      "Applied Mathematics",
    );
    const mathHndId = await upsertProgram(
      ctx,
      mathDeptId,
      "HND Applied Mathematics",
    );
    const mathHnd200 = await upsertAcademicClass(
      ctx,
      mathHndId,
      200,
      "REGULAR",
    ...
    const engId = await upsertFaculty(
      ctx,
      institution._id,
      "Faculty of Engineering",
    );
    const mechDeptId = await upsertDepartment(
      ctx,
      engId,
      "Mechanical Engineering",
    );
    const mechHndId = await upsertProgram(
      ctx,
      mechDeptId,
      "HND Mechanical Engineering",
    );
    const mechHnd200 = await upsertAcademicClass(
      ctx,
      mechHndId,
      200,
      "REGULAR",
    ...
    const bmsId = await upsertFaculty(
      ctx,
      institution._id,
    ...
    const acctHnd200Regular = await upsertAcademicClass(
      ctx,
      acctHndId,
      200,
      "REGULAR",
...
const COURSES: CourseSeed[] = [
  ...
  },
  {
    ...
    sections: [
      {
        divisionLabel: "A",
        lecturer: "Demo Lecturer 3",
        days: ["Tuesday", "Thursday"],
        time: "09:00 – 11:00",
        venue: "ICT Block, Lab 3",
      },
      {
        divisionLabel: "B",
        lecturer: "Demo Lecturer 4",
        days: ["Tuesday", "Thursday"],
        time: "13:00 – 15:00",
        venue: "ICT Block, Lab 3",
      },
  ...
  },
  {
    ...
    sections: [
      {
        divisionLabel: "A",
        lecturer: "Demo Lecturer 5",
        days: ["Wednesday"],
        time: "13:00 – 15:00",
        venue: "Block C, Room 5",
      },
    ],
  },
  {
    ...
    sections: [
      {
        lecturer: "Demo Lecturer 6",
        days: ["Friday"],
        time: "09:00 – 12:00",
        venue: "ICT Block, Lab 1",
      },
    ],
  },
  ...
  // provably exercised (these must NOT show up for the demo student).
  {
    ...
    sections: [
      {
        divisionLabel: "A",
        lecturer: "Demo Lecturer 7",
        days: ["Tuesday"],
        time: "08:00 – 10:00",
        venue: "ICT Block, Lab 2",
      },
    ],
  },
  {
    ...
    sections: [
      {
        lecturer: "Demo Lecturer 8",
        days: ["Thursday"],
        time: "14:00 – 16:00",
        venue: "ICT Block, Lab 4",
      },
    ],
  },
  {
    ...
    sections: [
      {
        divisionLabel: "A",
        lecturer: "Demo Lecturer 9",
        days: ["Tuesday", "Thursday"],
        time: "08:00 – 10:00",
        venue: "Engineering Block, Workshop 1",
      },
    ],
  },
];
...
export const seedCourses = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const course of COURSES) {
      const { academicClass } = await resolveClass(
        ctx,
        course.faculty,
        course.department,
        course.program,
      ...
      for (const section of course.sections) {
        ...
        if (section.divisionLabel !== undefined) {
          const divisions = await ctx.db
            .query("divisions")
            .withIndex("by_academicClassId", (q) =>
              q.eq("academicClassId", academicClass._id),
            )
            .collect();
          const division = divisions.find(
            (row) => row.label === section.divisionLabel,
          );
          if (division === undefined) {
            throw new Error(
              `Missing division ${section.divisionLabel} for ${course.code} — run seedHierarchy first`,
            );
          }
...
export const seedDemoStudent = internalAction({
  ...
  handler: async (ctx): Promise<Id<"users">> => {
    let userId = await ctx.runQuery(internal.users.findUserIdByEmail, {
...
export const seedDemoStudentData = internalMutation({
  ...
  handler: async (ctx, { userId }) => {
    ...
    if (divisionA === undefined) {
      throw new Error(
        "Missing Division A for the demo class — run seedHierarchy first",
      );
    }
    ...
    const semester = await requireActiveSemester(ctx);
    const cs301 = await findCourseByCode(
      ctx,
      semester._id,
      academicClass._id,
      "CS 301",
    );
    const cs305 = await findCourseByCode(
      ctx,
      semester._id,
      academicClass._id,
      "CS 305",
...
type CourseActivitySeed = Omit<
  Doc<"courseActivities">,
  "_id" | "_creationTime" | "studentId"
>;
...
export const seedActivities = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const requireCourse = async (code: string) => {
      const course = await findCourseByCode(
        ctx,
        semester._id,
        academicClass._id,
        code,
    ...
    const activities: CourseActivitySeed[] = [
      {
        ...
        status: "PENDING",
        notes:
          "Covers chapters 4–6: normalization forms (1NF–3NF), ER-to-relational mapping. Closed book, 30 minutes.",
      },
      {
        courseId: cs305._id,
        title: "Assignment 3",
        activityType: "ASSIGNMENT",
        dueDate: atOffset(0, 23, 59),
        priority: "IMPORTANT",
        status: "PENDING",
      },
      {
    ...
    const existingKeys = new Set(
      existing.map((row) => `${row.courseId}:${row.title}`),
    );
    for (const activity of activities) {
      if (existingKeys.has(`${activity.courseId}:${activity.title}`)) continue;
      await ctx.db.insert("courseActivities", {
...
export const seedDemoAlerts = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const requireCourseActivity = async (courseCode: string, title: string) => {
      ...
      if (course === null)
        throw new Error(`Run seedCourses first (missing ${courseCode})`);
      const rows = await ctx.db
        .query("courseActivities")
        .withIndex("by_studentId", (q) => q.eq("studentId", demoUser._id))
        .collect();
      const activity = rows.find(
        (row) => row.courseId === course._id && row.title === title,
      );
      if (activity === undefined)
        throw new Error(`Run seedActivities first (missing ${title})`);
    ...
    const requirePersonalReminder = async (title: string) => {
      const rows = await ctx.db
        .query("personalReminders")
        .withIndex("by_userId_and_semesterId", (q) =>
          q.eq("userId", demoUser._id).eq("semesterId", semester._id),
        )
        .collect();
      const reminder = rows.find((row) => row.title === title);
      if (reminder === undefined)
        throw new Error(
          `Run seedDemoStudent first (missing reminder ${title})`,
        );
      return reminder;
    };
    const requireSemesterActivity = async (title: string) => {
      ...
      if (event === undefined)
        throw new Error(`Run seedActivities first (missing event ${title})`);
      return event;
    };
    ...
    const portfolioDraft = await requireCourseActivity(
      "CS 320",
      "Portfolio Draft",
    );
    const studySession = await requirePersonalReminder(
      "Study DB Normalization",
    );
    const dataBundle = await requirePersonalReminder("Buy data bundle");
    const careerFair = await requireSemesterActivity("Career Fair");
    const midSemExams = await requireSemesterActivity(
      "Mid-semester exams begin",
    );
...
export const seedAll = internalAction({
  ...
  handler: async (ctx, { iAmSure }) => {
    ...
    console.warn(
      "[seed] Running seedAll — inserts demo/dev data. Never run this against a real production deployment.",
    );
    ...
    console.warn(
      `[seed] Done. Demo student: ${DEMO_STUDENT.authEmail} / ${DEMO_STUDENT.password}`,
    );
    console.warn(
      `[seed] Done. Demo admin: ${DEMO_ADMIN.authEmail} / ${DEMO_ADMIN.password}`,
    );
  },
});
